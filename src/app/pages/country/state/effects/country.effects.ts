    ...
    return this.actions$.pipe(
      ...
      switchMap((action) => {
        ...
        if (this.cache.has(serializedAction)) {
          this.cache.get(serializedAction).borders
            ? this.store.dispatch(
              getBorderCountriesAction({
                codes: this.cache.get(serializedAction).borders,
              })
            )
            : this.store.dispatch(getBorderCountriesRefresh());
          return of(
            getCountrySuccess({ country: this.cache.get(serializedAction) })
          );
        } else {
          return this.countriesService
            .getCountryByFullText(action.countryName)
            .pipe(
              map((res) => {
                this.cache.set(serializedAction, res[0]);
                res[0].borders
                  ? this.store.dispatch(
                    getBorderCountriesAction({ codes: res[0].borders })
                  )
                  : null;
    ...
    return this.actions$.pipe(
      ...
      switchMap((action) => {
        ...
        if (this.cache.has(serializedAction)) {
          return of(
            getBorderCountriesSuccess({
              countries: this.cache.get(serializedAction),
            })
          );
        } else {
