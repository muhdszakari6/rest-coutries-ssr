...
import { countryRootInitalState, TESTING_CONSTANTS } from 'src/app/shared/constants/test-constants';
...
describe('countryReducer', () => {
  ...
  describe('Known actions', () => {
    ...
    it('should change country state when getCountrySuccess is dispatched', () => {
      ...
      const action = getCountrySuccess({ country: COUNTRIES[1] as unknown as Country });
      const state = fromReducer.countryReducer(initialState, action);
      expect(state).toEqual(newState);
      expect(state).not.toBe(newState);
    });
    ...
    it('should change borderCountries state when getBorderCountriesSuccess is dispatched', () => {
      ...
      const action = getBorderCountriesSuccess({ countries: COUNTRIES as unknown as Country[] });
      const state = fromReducer.countryReducer(initialState, action);
      expect(state).toEqual(newState);
      expect(state).not.toBe(newState);
    });
  ...
  describe('Selectors', () => {
    it("should select the country", () => {
      const result = fromReducer.getCountry.projector(countryRootInitalState.country);
      expect(result).toBe(countryRootInitalState.country.country);
    });
    it("should select loading", () => {
      const result = fromReducer.getLoading.projector(countryRootInitalState.country);
      expect(result).toBe(countryRootInitalState.country.loading);
    });
    it("should select error", () => {
      const result = fromReducer.getError.projector(countryRootInitalState.country);
      expect(result).toBe(countryRootInitalState.country.error);
    });


    it("should select the border countries", () => {
      const result = fromReducer.getBorderCountriesSelector.projector(countryRootInitalState.country);
      expect(result).toBe(countryRootInitalState.country.borderCountries);
    });
    it("should select  border countries loading", () => {
      const result = fromReducer.getLoading.projector(countryRootInitalState.country);
      expect(result).toBe(countryRootInitalState.country.borderCountriesLoading);
    });
  })
